...
{
    [Serializable]
    public class GameUserViewDAO : AEDBGencTakimDataBaseEntity.Entities.GameUserViewBase
    {
        ...
        internal string Save()
        {
            ...
            if (this.Id == 0)
            {
                sqlcum = "Insert INTO [GameUserView](" + fieldsName + ")Values(" + fieldsValue + ")";

                DatabaseOperations.ParameterOperation(sqlcum, sqlparam);
                this.Id = Convert.ToInt32(DatabaseOperations.dtb("select max(Id) from [GameUserView]").Rows[0][0]);
                return "1";
            }
            else
            {
                sqlcum = "UPDATE [GameUserView] SET " + fieldsName + " where Id =" + this.Id;
                DatabaseOperations.ParameterOperation(sqlcum, sqlparam);
                return "2";
            }
        ...
        internal GameUserViewDAO Select(string sql_, params SqlParameter[] paramss)
        {

            GameUserViewDAO entity = new GameUserViewDAO();
        ...
        internal List<GameUserViewDAO> DataSource(string sql_, params SqlParameter[] paramss)
        {

            List<GameUserViewDAO> list = new List<GameUserViewDAO>();
            DataTable dt;
            if (paramss == null)
